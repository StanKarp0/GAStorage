...
def mut_individual(individual, indpb: float):
    ...
    order, = tools.mutShuffleIndexes(ind_arr[:, 1], indpb)
    rotated, = tools.mutFlipBit(ind_arr[:, 0], indpb)

    for index, position, rot in zip(ind_arr[:, 2], order, rotated):
        individual[index] = Package(rot, position, index)
